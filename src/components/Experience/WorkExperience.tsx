...
const experiences = [
  {
    ...
    responsibilities:
      "At 3P Learning, we specialize in developing web applications that leverage play to facilitate children's learning. My" +
  ...
  },
  {
    ...
    company: "3P Learning Ltd",
    responsibilities:
      "• Debugging and maintenance of applications under the Mathletics product." +
  ...
  },
  {
    ...
    responsibilities:
      "• Built and maintained CMS using WordPress." +
  ...
  //   responsibilities:
  //     "• Built and maintained various WordPress websites." +
  //     "<br/>• Integrated various plugins in accordance with customer’s needs." +
...
export default function WorkExperience() {
  return experiences.map((each, index) => {
    const even = (index + 1) % 2 === 0;
    const { date, role, company, responsibilities } = each;
    return (
      <div
        className={`md:w-[50%] relative px-10 pt-5 pb-10
      ...
      >
        <div className={"bg-gray-200 p-4 rounded-md"} key={date}>
          <div className="text-left text-accent-color-0">{date}</div>
          <h2 className="font-bold py-3 text-2xl text-black tracking-wide">
            {role} | {company}
          </h2>
          <ReadMore>
            <p
